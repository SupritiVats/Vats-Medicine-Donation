...
receivedexpress.get("/signup", function (req, res) {
  ...
  dbref.query(
    ...
    function (err, result) {
      console.log(
        req.query.txtEmail + " " + req.query.txtPass + " " + req.query.utype
      );
      if (err) {
        console.log(err);
      } else {
...
receivedexpress.get("/login", function (req, res) {
  dbref.query(
    ...
    [req.query.loginEmail, req.query.loginPassword],
    function (err, result) {
...
receivedexpress.get("/settings", function (req, res) {
  ...
  dbref.query(
    "select * from users where email=? and pwd=?",
    [req.query.txtUEmail, req.query.txtUPass],
    function (err, result) {
      if (err) res.send(err);
      else if (result.length == 1) {
        dbref.query(
          "update users set pwd=? where email =?",
          [req.query.txtNewPass, req.query.txtUEmail],
          function (err, result) {
            if (err) {
              res.send(err);
...
receivedexpress.get("/chkEmailKuch", function (req, resp) {
  ...
  dbref.query(
    ...
    function (err, resultAryOfObjects) {
      if (err) resp.send(err);
      else if (resultAryOfObjects.length == 0) {
        resp.send("Avilable....");
      } else resp.send("Already Occupied....");
    }
  );
});
//=============================json===============================================
...
receivedexpress.post("/avail-medicine", function (req, resp) {
  ...
  dbref.query(
    "insert into medicines values(?,?,?,?,?,?,?,?,current_date())",
    dataAry,
    function (err, result) {
      if (err) resp.send(err);
      else resp.send("Inserted Successfully..");
...
receivedexpress.get(
  ...
  function (
    ...
    console.log(req.query.txtEmail);
    dbref.query(
      "select * from nprofile where emailid=?",
      [req.query.txtNEmail],
      function (err, resultAryOfObjects) {
        if (err) resp.send(err);
        else resp.send(resultAryOfObjects);
...
receivedexpress.post("/Nprofile-update", function (req, resp) {
  ...
  // update profile set addr=?, state=?, tech=? , branch=?, profilepic=? where email=?
  dbref.query(
    "update nprofile set name=?, mobile=?, address=? , city=?, prooftype=?, timings=?, proofpic=?, profilepic=? where emailid=?",
    dataAry,
    function (err, result) {
      if (err) resp.send(err);
      else resp.send("Inserted Successfully..");
...
receivedexpress.get("/needysettings", function (req, res) {
  ...
  dbref.query(
    "select * from users where email=? and pwd=?",
    [req.query.txtNEmail, req.query.txtOPass],
    function (err, result) {
      if (err) res.send(err);
      else if (result.length == 1) {
        dbref.query(
          "update users set pwd=? where email =?",
          [req.query.txtNPass, req.query.txtNEmail],
          function (err, result) {
            if (err) {
              res.send(err);
            } else res.send("updated successfully..-.");
...
receivedexpress.get("/delete-donor", function (req, res) {
  dbref.query(
    "delete from dprofile where emailid=?",
    [req.query.emailidX],
    function (err, result) {
      if (err) res.send(err);
      else if (result.affectedRows == 0) res.send("Invalid Id");
      else res.send("Record Deleted Successfulllllyyyyy");
    }
  );
});
...
receivedexpress.get("/delete-needy", function (req, res) {
  dbref.query(
    "delete from nprofile where emailid=?",
    [req.query.emailidX],
    function (err, result) {
      if (err) res.send(err);
      else if (result.affectedRows == 0) res.send("Invalid Id");
...
receivedexpress.get("/fetchAllMedicine", function (req, resp) {
  dbref.query(
    "select distinct medicine from medicines inner join dprofile on medicines.emailid=dprofile.emailid where dprofile.city=?",
    [req.query.city],
    function (err, resultAryOfObjects) {
      if (err) resp.send(err);
      else {
...
receivedexpress.get("/fetchdonor", function (req, resp) {
  dbref.query(
    "select * from dprofile inner join medicines on dprofile.emailid=medicines.emailid where dprofile.city=? and medicines.medicine=?",
    [req.query.city, req.query.med],
    function (err, resultAryOfObjects) {
      if (err) resp.send(err);
      else resp.send(resultAryOfObjects);
    }
...
receivedexpress.get("/fetchMedicineRecords", function (req, res) {
  dbref.query(
    "select * from medicines where emailid=?",
    [req.query.email],
    function (err, result) {
      if (err) {
        res.send(err);
...
receivedexpress.get("/unlist-the-medicine", function (req, res) {
  dbref.query(
    "delete from medicines where emailid=?",
    [req.query.emailidX],
    function (err, result) {
      if (err) res.send(err);
      else if (result.affectedRows == 0) res.send("Invalid Id");
